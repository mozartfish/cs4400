...
void eval(char *cmdline)
{
  ...
  if (!builtin_cmd(argv1))
  {
    ...
    // determine if its a background or foreground process
    if (!bg)
    {
...
void do_bg(int jid)
{
  // search through the jobs to find the job id that matches jid and change the state from ST to BG
  // #define UNDEF 0 /* undefined */
  // #define FG 1    /* running in foreground */
  // #define BG 2    /* running in background */
  // #define ST 3    /* stopped */
