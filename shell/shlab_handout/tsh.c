...
void eval(char *cmdline)
{
  ...
  // int fds[2]; // array for handling the file descriptor
  //             // fds[0] is read end
  //             // fds[1] is write end
  ...
  {
    // Block SIGCHLD signal information goes here
    // child runs the job
    if ((pid = fork()) == 0)
    {
      ...
      //    // add the job. Offset by 1 to match the enum values for the different states
...
void do_bg(int jid)
{
  ...
  // if (!exist_bg)
  // {
  //   printf("A background job was not found");
  // }
  return;
}
